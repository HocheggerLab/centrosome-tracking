...
_fig_size_square = (3, 3)
_fig_size_small_square = (2.5, 2.5)
_fig_size_rect = (4, 2.47)
_fig_size_small_rect = (2, 1.24)
...
def dist_stlc(df, dfc):
    _conds = ['1_N.C.', '1_P.C.']
    dfs, conds, colors = sorted_conditions(df, _conds)
    dfc, conds, colors = sorted_conditions(dfc, _conds)

    # sp.render_tracked_centrosomes(parameters.data_dir + 'centrosomes.nexus.hdf5', 'pc', 'run_114', 2)
    # img_fnames = [os.path.join(parameters.data_dir + 'out', 'run_114_N02_F%03d.png' % f) for f in range(20)]
    ...
    with PdfPages(parameters.data_dir + 'out/stlc.pdf') as pdf:
        ...
        fig.clf()
        fig.set_size_inches(_fig_size_small_square)
        ax = fig.gca()
        ...
        fig = plt.figure(dpi=_dpi)
        fig.clf()
        fig.set_size_inches(_fig_size_rect)
        ax = fig.gca()
...
def msd_stlc(df, dfc):
    ...
    with PdfPages(parameters.data_dir + 'out/msd-stlc.pdf') as pdf:
        ...
        sns.tsplot(data=df_msd,
                   linestyle='-', condition='condition',
                   lw=_indvlw,  # err_style='ci_bars', ci='sd',
        ...
        fig.clf()
        ax1 = fig.gca()
        sp.msd(dfs[dfs['condition'] == names['1_N.C.']], ax1, ylim=[0, 300], color=sp.SUSSEX_COBALT_BLUE)
        ...
        sp.msd(dfs[dfs['condition'] == names['1_P.C.']], ax2, ylim=[0, 300], color=sp.SUSSEX_COBALT_BLUE)
        ax2.set_ylabel('MSD [$\mu m^2$]')
        ...
        sns.tsplot(data=df_msd2,
                   linestyle='-', condition='condition', lw=_indvlw,
                   time=time, value='msd', unit='indv', estimator=np.nanmean, ax=ax)
        ax.set_xticks(np.arange(0, df_msd['Time'].max(), 20.0))
        ax.legend(title=None, loc='upper left')
        ax.set_ylim([0, 120])
        ...
        sns.tsplot(data=df_msd1,
                   linestyle='-', condition='condition', color=sp.SUSSEX_COBALT_BLUE,
                   lw=_indvlw, err_style=['unit_traces'], err_kws=_err_kws,
                   time=time, value='msd', unit='indv', estimator=np.nanmean, ax=ax)
        sns.tsplot(data=df_msd2,
                   linestyle='-', condition='condition',
...
def stlc_selected_track(df, mask):
...
def stlc_mother_daughter(df, dfc):
    ...
    with PdfPages(parameters.data_dir + 'out/fig1-mother.pdf') as pdf:
        ...
        with sns.color_palette([sp.SUSSEX_CORAL_RED, sp.SUSSEX_COBALT_BLUE]):
            sns.tsplot(data=df_msd, lw=_indvlw, err_style=['unit_traces'], err_kws=_err_kws,
...
def dynein(df, dfc):
    with PdfPages(parameters.data_dir + 'out/dynein.pdf') as pdf:
        ...
        #          NEXT PAGE
        # ---------------------------
        fig = plt.figure(figsize=_fig_size_square, dpi=_dpi)
        fig.clf()
        ...
        sns.tsplot(data=dfcs, time='Time', value='DistCentr', unit='indv', condition='condition',
                   estimator=np.nanmean, ax=ax, lw=_indvlw, color=sp.SUSSEX_COBALT_BLUE,
                   err_style=None, err_kws=_err_kws)
        _conds = ['1_Dynei']
        dfcs, conds, colors = sorted_conditions(dfc, _conds)
        sns.tsplot(data=dfcs, time='Time', value='DistCentr', unit='indv', condition='condition',
                   estimator=np.nanmean, ax=ax, lw=_indvlw, color=sp.SUSSEX_CORAL_RED,
        ...
        with sns.color_palette([sp.SUSSEX_CORAL_RED, sp.SUSSEX_COBALT_BLUE]):
            sns.tsplot(data=dfcs, time='Time', value='DistCentr', unit='indv', condition='condition',
                       estimator=np.nanmean, ax=ax3, lw=_indvlw,
                       err_style=['unit_traces'], err_kws=_err_kws)
        ...
        sp.msd(df[df['condition'] == names['1_DIC']], ax5, ylim=[0, 400], color=sp.SUSSEX_COBALT_BLUE)
    ...
    with PdfPages(parameters.data_dir + 'out/actin.pdf') as pdf:
        ...
        for _cond in ['1_FAKI', '1_CyDT', '1_Bleb']:
            ...
            with sns.color_palette([sp.SUSSEX_COBALT_BLUE]):
                sns.tsplot(data=dfcs, time='Time', value='DistCentr', unit='indv', condition='condition',
                           estimator=np.nanmean, ax=ax, lw=_indvlw,
                           err_style=['unit_traces'], err_kws=_err_kws)
                ax.set_xlabel('time prior contact [min]')
                ax.set_ylabel('Distance [um]')
                ax.legend(title=None, loc='upper left')
...
if __name__ == '__main__':
    ...
    dist_stlc(df_m, dfcentr)
    msd_stlc(df_m, dfcentr)
    dynein(df_m, dfcentr)
    kinesin(df_m, dfcentr)
    actin(df_m, dfcentr)
